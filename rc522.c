...
static uint8_t* rc522_anticoll(rc522_handle_t rc522)
{
    ...
    if(res_n != 5) { // all cards/tags serial numbers is 5 bytes long (?)
        free(result);
        return NULL;
    }
...
static uint8_t rc522_select_tag(rc522_handle_t rc522, uint8_t* uid)
{
    ...
    uint8_t res_data_n;
    uint8_t buf[] = { 0x93, 0x70, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    for(uint8_t i = 0; i < 5; i++) {
      buf[i+2] = uid[i];
    }
    uint8_t* crc = rc522_calculate_crc(rc522, buf, 7);
    buf[7] = crc[0];
    buf[8] = crc[1];
    free(crc);
    res_data = rc522_card_write(rc522, 0x0C, buf, 9, &res_data_n);
    uint8_t ret = res_data[0];
    free(res_data);
    return ret;
}
...
static void rc522_auth(rc522_handle_t rc522, uint8_t* uid)
{
    ...
    uint8_t buf[] = { 0x60, 0x08, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00 };
    for(uint8_t i = 0; i < 4; i++) {
      buf[i+8] = uid[i];
    }
    res_data = rc522_card_write(rc522, 0x0E, buf, 12, &res_data_n);
    if (!res_data){
      free(res_data);
    }
}
...
static uint64_t rc522_read_tag(rc522_handle_t rc522)
{
    ...
    uint8_t buf[] = { 0x30, 0x08, 0x00, 0x00 };
...
static void rc522_write_tag(rc522_handle_t rc522, uint8_t write_data)
{
    ...
    uint8_t buf[] = { 0xA0, 0x08, 0x00, 0x00 };
    uint8_t* crc = rc522_calculate_crc(rc522, buf, 2);
    buf[2] = crc[0];
    buf[3] = crc[1];
    free(crc);
    ...
    uint8_t buff[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 , 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 , 0x00};
    buff[0] = write_data;
    //for(uint8_t i = 0; i < 1; i++) {
    //  buff[i] = write_data[i];
    //}
    crc = rc522_calculate_crc(rc522, buff, 16);
    buff[16] = crc[0];
    buff[17] = crc[1];
    free(crc);
    res_data = rc522_card_write(rc522, 0x0C, buff, 18, &res_data_n);
    //if (res_data_n == 4)
    //  flag |= 1<<16;
    //if ((res_data[0] & 0x0F )== 0x0A)
    //  flag |= 1<<24;
...
static void rc522_task(void* arg)
{
    ...
    while(rc522->running) {
        ...
        if(res_data != NULL) {
            ...
            serial_no_array = rc522_anticoll(rc522);
            if(! serial_no_array) {
                ...
                if ((tag.write_data != prev_write) || (tag.read_data != prev_read)
                    || (tag.write_mode != prev_write_mode) || (tag.serial_number != prev_serial)){
